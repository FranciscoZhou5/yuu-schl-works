...
export const badgeColorHandler: { [K in BadgeColors]: string } = {
...
export default function Badge({ children, color = "default" }: IBadgeProps) {
  return (
    <span className={classNames("whitespace-nowrap text-xs font-medium mr-2 px-2.5 py-0.5 rounded min-w-[45px]", badgeColorHandler[color])}>
      {children}
    </span>
  );
}
